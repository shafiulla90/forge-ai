...
async function run() {
  ...
  try {
    ...
    console.log('Querying Apex Triggers...');
    const triggers = await conn.tooling.query("SELECT Name, Body, TableEnumOrId FROM ApexTrigger");
    console.log('Total triggers found:', triggers.records.length);
    triggers.records.forEach(tr => {
      console.log(`Trigger: ${tr.Name} on ${tr.TableEnumOrId}`);
      if (tr.Name === 'AccountTrigger' || tr.Name === 'ContactTrigger') {
        console.log('Body:', tr.Body);
      }
    });
