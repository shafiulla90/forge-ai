...
async function run() {
  ...
  console.log('Pre-populating plan for SCRUM-13...');

  // 1. Define the 6-step plan
  const plan = {
...
<Flow xmlns="http://soap.sforce.com/2006/04/metadata">
  ...
  const { data: updatedDep, error } = await supabase
    .from('deployments')
    .update({
  ...
  if (error) {
    console.error('Error pre-populating deployment:', error);
  } else {
    console.log('Successfully pre-populated deployment record in Supabase:');
    console.log(JSON.stringify(updatedDep, null, 2));
  }
}
run();
